...
DMDData::DMDData(IntITKType d, StrITKType inputDirec){
...
void DMDData::interpolate3D ( const OrientedImageType::Pointer input, StrITKType caseID)
{
    ...
    for(int l = CRAN_SART; l < CAUD_SART; l++){
        ...
        for(indexOutput[2] = 0; indexOutput[2] < roundRange; indexOutput[2]++) { // start from the 3rd slice
            for(indexOutput[1] = 1; indexOutput[1] < (int)size[1] - 1; indexOutput[1]++) {
              ...
              //   interpolatedVal = upperVal * upperFactor + bottomVal * bottomFactor;
    ...
    for(indexOutput[2] = 1; indexOutput[2] < (int)size[2] - 1; indexOutput[2]++) {
        for(indexOutput[1] = 1; indexOutput[1] < (int)size[1] - 1; indexOutput[1]++) {
            ...
            //    if( indexOutput[2] >= (int) (size[2] / 2 - 2) && indexOutput[2] <= (int) (size[2] / 2 + 2) ){
...
/*
    ...
    outputfilename = caseID + "_intervol.nrrd";
    dataWriter(writer, outputfilename);
    //output manually segmented muscle volume without reference slices
    writer->SetInput( referenceVolume );
    outputfilename = caseID + "_referencevol.nrrd";
    dataWriter(writer, outputfilename);
    //output interpolated semit volume without reference slices
    writer->SetInput( segSemit );
    outputfilename = caseID + "_outputvolumeSemit.nrrd";
    dataWriter(writer, outputfilename);
    //output manually segmented semit volume without reference slices
    writer->SetInput( referenceSemit );
    outputfilename = caseID + "_referencevolumeSemit.nrrd";
    dataWriter(writer, outputfilename);
    //output interpolated rec fem volume without reference slices
    writer->SetInput( segRecFem );
    outputfilename = caseID + "_outputvolumeRecFem.nrrd";
    dataWriter(writer, outputfilename);
    //output manually segmented rec fem volume without reference slices
    writer->SetInput( referenceRecFem );
    outputfilename = caseID + "_referencevolumeRecFem.nrrd";
    dataWriter(writer, outputfilename);
    //output interpolated cran sart volume without reference slices
    writer->SetInput( segCranSart );
    outputfilename = caseID + "_outputvolumeCranSart.nrrd";
    dataWriter(writer, outputfilename);
    //output manually segmented cran sart volume without reference slices
    writer->SetInput( referenceCranSart );
    outputfilename = caseID + "_referencevolumeCranSart.nrrd";
    dataWriter(writer, outputfilename);
*/
   // exit(1);
