...
TextureFeatureCal::TextureFeatureCal(){

    char *muscle[] = { "MUSCLES", "CRAN_SART", "REC_FEM", "BIC_FEM", "GRACILIS", "SEMI_MEM", "ADDUCTOR", "SEMIT", "VAS_LAT", "VAS_INT", "VAS_MED", "CAUD_SART"};
    muscleName = muscle;
    meanCran = 0;
     meanRect = 0;
     meanSemit = 0;
...
void TextureFeatureCal::calCooccurrenceFeatures( DMDData::FITKType &EntropyTmp, DMDData::FITKType &EnergyTmp, DMDData::FITKType &ContrastTmp, DMDData::FITKType &HomoGeneityTmp, float  cooccurrencematrix[CO_OCCURRENCE_LEVEL][CO_OCCURRENCE_LEVEL])
{
    ...
    // std::cout << "number of run: " << nr << std::endl;
    std::string featurefilename = "../data/voiCOM.txt" ;
    std::ofstream efile( featurefilename.c_str() , std::ios::app );
...
//////////////////////////////////////////////////////////////////////////
void TextureFeatureCal::runlengthFeat( DMDData::OrientedImageType::Pointer erodemask, DMDData::OrientedImageType::Pointer data, std::string featurefilename)
{
...
//////////////////////////////////////////////////////////////////////////
void TextureFeatureCal::runlengthFeat3D( DMDData::OrientedImageType::Pointer erodemask, DMDData::OrientedImageType::Pointer data, std::string featurefilename, FITKType voxelSize)
{
...
//////////////////////////////////////////////////////////////////////////
void TextureFeatureCal::runlengthFeat2DROI( DMDData::OrientedImageType::Pointer erodemask, DMDData::OrientedImageType::Pointer data, std::string featurefilename, FITKType voxelSize)
{
 ...
 //   std::cout << "size of interval: " << intervalX << "  " << intervalY << "  " << intervalZ << std::endl;
  //  std::cout << "size of interval: " << spacing[0] << "  " << spacing[1] << "  " << spacing[2] << std::endl;
    ...
    if(labelMin == 0 && labelMax == 0) {
        constErodeMaskIterator.GoToBegin();
        labelMin = labelMax = constErodeMaskIterator.Get();
        for ( constErodeMaskIterator.GoToBegin(); !constErodeMaskIterator.IsAtEnd(); ++constErodeMaskIterator ) {
            if (constErodeMaskIterator.Get() < labelMin)
                labelMin = constErodeMaskIterator.Get();
            if (constErodeMaskIterator.Get() > labelMax)
                labelMax = constErodeMaskIterator.Get();
        }
        labelMin++;
    }
    // identify minum and maxmum intensity of the image
            ...
            for (int n = 0; n < size[1]; n += round(intervalY * 0.5)) {
                for (int r = 0; r < size[0]; r += round(intervalX * 0.5)) {
                    ...
                    if(haveMuscleFlag) {
                        ...
                        std::string matrixfilename = "../data/matrix.txt";
   ...
   //     std::cout << "size of " << muscleName[p] << ": " << nroi << std::endl;
    ...
    std::ofstream efile( featurefilename.c_str() , std::ios::app );
    efile << "     ";
    for ( int p = labelMin ; p <= labelMax; p++ ) {
        efile << "Label_"<< p << "  " ;
    }
    efile << "\nEntropy";
    for ( int p = labelMin ; p <= labelMax; p++ ) {
...
/*
//////////////////////////////////////////////////////////////////////////
void TextureFeatureCal::runlengthFeat3DROI( DMDData::OrientedImageType::Pointer erodemask, DMDData::OrientedImageType::Pointer data, std::string featurefilename, FITKType voxelSize, StrITKType caseID)
{
    //ROI based run length features
    int runlengthmatrix[NUMBER_MUSCLE + 1][RUN_LENGTH_LEVEL][RUN_INTENSITY_LEVEL];
    int tempIntensity = 0, tempLength = 0, intensityMax = 0, intensityMin = 0, intensityMaxCut = 0, intensityMinCut = 0, histogram[HISTOGRAM_LIMIT] = {0};
...
void TextureFeatureCal::histogramFeat3DROI( DMDData::OrientedImageType::Pointer mask, DMDData::OrientedImageType::Pointer erodemask, DMDData::OrientedImageType::Pointer data, float voxelSize, std::string featurefilename, bool volOutputMark )
{
    ...
    if(sdCaud)
        ...
        if(volOutputMark){
        efile << "     ";
        for (int i = labelMin; i <= labelMax; i++){
            efile << "Label_" << i << "  ";
        }
        efile << "\nmean          ";
    }
    ...
    efile << "skewness" << "  " << skewCran << "  " << skewRect << "  " << skewBic << "  " << skewGracilis << "  " << skewSemimem << "  " << skewAdductor << "  " << skewSemit << "  " << skewVaslat << "  " << skewVasint << "  " << skewVasmed << "  " << skewCaud << "\n";
    efile << "kortness" << "  " << kortCran << "  " << kortRect << "  " << kortBic << "  " << kortGracilis << "  " << kortSemimem << "  " << kortAdductor << "  " << kortSemit << "  " << kortVaslat << "  " << kortVasint << "  " << kortVasmed << "  " << kortCaud << "\n";
    efile << "entropy"  << "  " << entropyCran << "  " << entropyRect << "  " << entropyBic << "  " << entropyGracilis << "  " << entropySemimem << "  " << entropyAdductor << "  " << entropySemit << "  " << entropyVaslat << "  " << entropyVasint << "  " << entropyVasmed << "  " << entropyCaud << "\n";
#endif
    efile.close();
