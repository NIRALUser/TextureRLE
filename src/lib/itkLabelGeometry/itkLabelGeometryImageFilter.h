 ...
 * geometric features.
 * This filter enables the measurement of
 * geometric features of all objects in a labeled ND volume. This
 * labeled volume can represent, for instance, a medical image
 * segmented into different anatomical structures or a microscope
 * image segmented into individual cells. This filter is closely
 * related to the 
 * itkLabelStatisticsImageFilter, which measures statistics of image
 * regions defined by a labeled mask such as min, max, and mean
 * intensity, intensity standard deviation, and bounding boxes. This
 * filter, however, measures the geometry of the
 * labeled regions themselves.  
 * It calculates features similar to the regionprops command of Matlab.
 * The set of measurements that it
 * enables include: volume, centroid, eigenvalues, eigenvectors, axes
 * lenghts, eccentricity, elongation, orientation, bounding box,
 * oriented bounding box, and rotation matrix.  These features are
 * based solely on the labeled
 * mask itself.  It also calculates integrated intensity and weighted
 * centroid, which are measured on an intensity
 * image under the labeled mask. These features represent the set of currently
 * calculated features, but the framework of the filter is designed so
 * that it can be easily expanded to measure a wide variety of other
 * features. 
 */
...
public:
  ...
  void SetIntensityInput(TIntensityImage *input)
    {
  ...
  /** Get the label image */
  TIntensityImage * GetIntensityInput()
    {
